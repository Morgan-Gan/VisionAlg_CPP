...
namespace common_paralleltask
{
	...
	public:
		...
		void Run(common_paralleltask::Task<R()>&& task)
		{
		...
		void Run(P&& f)
		{
			...
			typedef typename common_paralleltask::Task<ReturnType()> T;
	...
	common_paralleltask::Task<std::vector<typename Range::value_type::return_type>()> WhenAll(Range& range)
	{
		...
		return common_paralleltask::Task<std::vector<ReturnType>()>(task);
	}
	...
	template<typename Range>
	common_paralleltask::Task<std::pair<int, typename Range::value_type::return_type>()> WhenAny(Range& range)
	{
		auto task = [&range]
		{
		...
		typedef typename Range::value_type::return_type ReturnType;
		return common_paralleltask::Task<std::pair<int, ReturnType>()>(task);
	}


	void TestWhenAll()
	{
		std::vector<common_paralleltask::Task<int()>> v =
		{
			common_paralleltask::Task<int()>([]()->int {PrintHead(); std::this_thread::sleep_for(std::chrono::seconds(1)); return 1; }),
			common_paralleltask::Task<int()>([]()->int {PrintHead(); return 2; }),
			common_paralleltask::Task<int()>([]()->int {PrintHead(); return 3; }),
			common_paralleltask::Task<int()>([]()->int {PrintHead(); return 4; })
		};
	...
	void TestWhenAny()
	{
		std::vector<common_paralleltask::Task<int()>> v =
		{
			common_paralleltask::Task<int()>([] {PrintHead(); std::this_thread::sleep_for(std::chrono::seconds(1)); return 1; }),
			common_paralleltask::Task<int()>([] {PrintHead(); return 2; }),
			common_paralleltask::Task<int()>([] {PrintHead(); return 3; }),
			common_paralleltask::Task<int()>([] {PrintHead(); return 4; })
		};
